...
const GooeyNav = ({
  ...
  const initialActiveIndex = items.findIndex(item => item.href === pathname) ?? 0;
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const newActiveIndex = items.findIndex(item => item.href === pathname);
    if (newActiveIndex !== -1 && newActiveIndex !== activeIndex) {
      setActiveIndex(newActiveIndex);
      const liEl = navRef.current?.querySelectorAll('li')[newActiveIndex];
      if (liEl) {
        updateEffectPosition(liEl);
      }
    }
      ...
      <style>
        {`
          :root {
            ...
            --color-3: hsl(var(--secondary-foreground));
            --color-4: hsl(var(--foreground));
          }
          .effect {
            position: absolute;
            opacity: 1;
          ...
          >
            {items.map((item, index) => (
              <li
                ...
                className={cn("gooey-nav-item rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent]", {
