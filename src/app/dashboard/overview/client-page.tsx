...
import { updateActualYield, YieldPredictionHistory } from '../actions';
...
interface DashboardClientPageProps {
    initialWeatherData: DashboardWeatherOutput | null;
    initialMarketData: MarketPriceAnalysisOutput | null;
    initialYieldHistory: YieldPredictionHistory[];
...
export default function DashboardClientPage({
  ...
  const [weatherData, setWeatherData] = useState<DashboardWeatherOutput | null>(initialWeatherData);
  const [marketData, setMarketData] = useState<MarketPriceAnalysisOutput | null>(initialMarketData);
  const [yieldHistory, setYieldHistory] = useState<YieldPredictionHistory[]>(initialYieldHistory);
  const [chatHistory, setChatHistory] = useState<Message[]>(initialChatHistory);
  
  const [isWeatherLoading, setIsWeatherLoading] = useState(!initialWeatherData);
  const [isMarketLoading, setIsMarketLoading] = useState(!initialMarketData);
